"""PHASE_3H — fetch the archived `jug05.asp` per-player pages (2005–2007).

`jug05.asp` is bsnpr.com's 2005-era player page: an "Estadísticas Jugador"
heading with the name, a Ciudad/Nacimiento/Posición bio table, and a
career-by-season stat table. Same identity-spine value as `jugador.asp`, but
it has no `?id=N` — pages are keyed only by an opaque `r2` token and an `e=`
roster city, so several URLs resolve to identical content.
...
gated tranches by capture year (2005 ~145, 2006 ~353, 2007 ~102 — each < 500).
PC6 single stream. Raw bytes written unmodified and never re-fetched (PC5).

Run: `python -m src.fetch_jug05 [--year 2005|2006|2007] [--limit N] [--force]`
     (`make fetch-jug05` runs every ungated year in sequence)
"""
...
OUT_DIR = REPO_ROOT / "data" / "raw" / "players" / "jug05"
MANIFEST = REPO_ROOT / "data" / "interim" / "fetch_manifest_jug05.csv"
MAX_TRANCHE = 500
PROGRESS_EVERY = 25


def _targets(year: str | None = None) -> list[dict]:
    """One record per distinct digest: the latest 200 capture of /jug05.asp.
    ...
    for r in rows:
        ...
        if parts.path.lower() != "/jug05.asp" or r[ix["statuscode"]] != "200":
            continue
...
def _fetch(targets: list[dict], session: requests.Session) -> list[dict]:
    ...
    for t in targets:
        ...
        got = _fetch_one(url, OUT_DIR / fname, {
            "tranche": "jug05", "digest": t["digest"], "wayback_timestamp": t["timestamp"],
...
def main() -> int:
    ...
    print(f"[jug05.asp] {all_n} distinct digests total; tranches: {', '.join(years)}")
    ...
    print(f"[jug05.asp] -> {OUT_DIR.relative_to(REPO_ROOT)} + {MANIFEST.relative_to(REPO_ROOT)}")
    return 0
